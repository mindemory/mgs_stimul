function A05_SubjectwiseALI(tfr_type, base_corr)
...
if strcmp(hostname, 'zod')
    fName.subjectALI                              = ['/d/DATD/datd/MD_TMS_EEG/EEGfiles/subjectALI_' tfr_type '.mat'];
end

subjectALI = [];
trl_mat = [];
if ~exist(fName.subjectALI, 'file')
    for subjID                              = subs
...
%                 pow_diff                    = ft_math(cfg, this_contra, this_ipsi);
                tcount                      = size(this_ipsi.powspctrm, 1);
                subjectALI                  = cat(1, subjectALI, this_ipsi.powspctrm);
                subjectALI                  = cat(1, subjectALI, this_contra.powspctrm);
                
                temp_trl_mat                = NaN(tcount*2, 5);
                ...
                temp_trl_mat(:, 4)          = ii;
                disp([subjID day])
                temp_trl_mat(:, 5)          = [setdiff(trl_idx.(ttype), trls_to_remove); setdiff(trl_idx.(ttype), trls_to_remove)];
                temp_trl_mat(:, 6)          = [ones(tcount, 1); zeros(tcount, 1)];
                trl_mat                     = [trl_mat; temp_trl_mat];
            end
        end
        clearvars POW;
    end
    subjectALI = squeeze(subjectALI);
    save(fName.subjectALI, 'subjectALI', 'trl_mat', '-v7.3')
else
    disp('Loading existing master ALI file')
    load(fName.subjectALI)
end
