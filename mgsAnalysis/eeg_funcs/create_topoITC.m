function create_topoITC(ITC, tidx, fidx, t_type, freqband)
...
in_type                                          = [t_type 'in'];
out_type                                         = [t_type, 'out'];
NTin_tfr                                         = ITC.NT.(in_type).all;
NTout_tfr                                        = ITC.NT.(out_type).all;
Tin_tfr                                          = ITC.T.(in_type).all;
Tout_tfr                                         = ITC.T.(out_type).all;
...
NT_pmax                                          = max(NTcontrast.itcspctrm(NT_idx, fidx, tidx), [], 'all', 'omitnan');
NT_pmin                                          = min(NTcontrast.itcspctrm(NT_idx, fidx, tidx), [], 'all', 'omitnan');
T_pmax                                           = max(Tcontrast.itcspctrm(T_idx, fidx, tidx), [], 'all', 'omitnan');
T_pmin                                           = min(Tcontrast.itcspctrm(T_idx, fidx, tidx), [], 'all', 'omitnan');
