function compare_condsITC(ITC, tidx, fidx, bl_type)
...
for i                                                        = 1:length(conditions)
    ...
    for k                                                    = 1:length(stim_types)
        ...
        if strcmp(condition, 'NT')
            temp_pow                                         = ITC.(condition).(t_types{1}).(stim_type).itcspctrm;
            this_pow                                         = reshape(squeeze(temp_pow), [1, size(temp_pow, 3), size(temp_pow, 4)]);
            ITC.(condition).(t_types{1}).(stim_type).label            = {'lol'};
            ITC.(condition).(t_types{1}).(stim_type).dimord           = 'chan_freq_time';
            ITC.(condition).(t_types{1}).(stim_type).itcspctrm        = this_pow;
            ...
            for j                                            = 1:length(t_types)
                t_type                                       = t_types{j};
                stim_type                                    = stim_types{k};
                temp_pow                                     = ITC.(condition).(t_type).(stim_type).itcspctrm;
                this_pow                                     = reshape(squeeze(temp_pow), [1, size(temp_pow, 3), size(temp_pow, 4)]);

                ITC.(condition).(t_type).(stim_type).label   = {'lol'};
                ITC.(condition).(t_type).(stim_type).dimord  = 'chan_freq_time';
                ITC.(condition).(t_type).(stim_type).itcspctrm   = this_pow;
                max_values                                   = [max_values, max(this_pow(:, fidx, tidx), [], 'all', 'omitnan')];
                min_values                                   = [min_values, min(this_pow(:, fidx, tidx), [], 'all', 'omitnan')];
            end
        end
...
TFRcontrast.NT                                   = ft_math(cfg, ITC.NT.(t_types{1}).contra, ITC.NT.(t_types{1}).ipsi);
TFRcontrast.T.(t_types{1})                       = ft_math(cfg, ITC.T.(t_types{1}).contra, ITC.T.(t_types{1}).ipsi);
TFRcontrast.T.(t_types{2})                       = ft_math(cfg, ITC.T.(t_types{2}).contra, ITC.T.(t_types{2}).ipsi);
...
subplot(3, 3, 1)
ft_singleplotTFR(cfg, ITC.NT.(t_types{1}).ipsi);
xlabel('Time (s)')
ylabel('Frequency (Hz)')
subplot(3, 3, 2)
ft_singleplotTFR(cfg, ITC.NT.(t_types{1}).contra);
xlabel('Time (s)')
ylabel('Frequency (Hz)')
subplot(3, 3, 4)
ft_singleplotTFR(cfg, ITC.T.(t_types{1}).ipsi);
xlabel('Time (s)')
ylabel('Frequency (Hz)')
subplot(3, 3, 5)
ft_singleplotTFR(cfg, ITC.T.(t_types{1}).contra);
xlabel('Time (s)')
ylabel('Frequency (Hz)')
subplot(3, 3, 7)
ft_singleplotTFR(cfg, ITC.T.(t_types{2}).ipsi);
xlabel('Time (s)')
ylabel('Frequency (Hz)')
subplot(3, 3, 8)
ft_singleplotTFR(cfg, ITC.T.(t_types{2}).contra);
xlabel('Time (s)')
ylabel('Frequency (Hz)')
