function A02_ControlEyeAnalysis(subjID, day, end_block, prac_status, only_edf2asc)
...
% Load taskMap for this day
load([p.phosphene '/taskMap_sub' p.subjID '_day06_antitype_mirror.mat']);
distractorHemi = arrayfun(@(x) taskMap(x).distractorHemi, 1:10, 'UniformOutput', false);
distractorHemiVector = cell2mat(distractorHemi(:));
...
if exist(ii_sess_saveName_mat, 'file') == 2
    ...
    if ~only_edf2asc
        ...
        ii_sess.distractorHemi = distractorHemiVector;
