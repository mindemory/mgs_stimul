...
for ii = 1:length(PhosphReport)
    ...
    [r_outer, r_inner, theta] = pixel2va(vc.mean(1),vc.mean(2),'ul'); % r in pixel
    ...
    %coords_all = [edge1; coords_outer; edge2; coords_inner];
    coords_all = [edge1; coords_outer; flip(edge2); flip(coords_inner)];
    coords_all = round(coords_all);
    
    [area_outer, border_outer] = analyzeDrawing(coords_all, tmsRtnTpy);
    [area_inner, border_inner] = analyzeDrawing(coords_inner, tmsRtnTpy);
